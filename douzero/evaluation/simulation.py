...
def evaluate(landlord, landlord_up, landlord_down, eval_data, num_workers):
    ...
    results_df = pd.DataFrame({
        'no': list(range(1,100)),  # 这里的 1 表示第一次运行的结果
        'landlord_model': 'douzero_ADP',  # 请将 'your_model_name' 替换为实际的模型名称
        'landlord_wp': num_landlord_wins / num_total_wins,
        'landlord_adp': num_landlord_scores / num_total_wins,
    })
    results_df.to_excel('results.xlsx', index=False, header=True, engine='openpyxl')